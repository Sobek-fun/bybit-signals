...
def _drop_strategy_labels(df: pd.DataFrame) -> tuple[pd.DataFrame, int]:
    drop_cols = [
        c
        for c in df.columns
        if c.startswith("strat_resolved_")
        or c.startswith("strat_prev_closed_")
        or c in {"strat_last_closed_is_sl", "strat_last_closed_is_tp"}
    ]
...
def _exp1(run_root: Path) -> str:
    ...
    df, dropped = _drop_strategy_labels(df)
...
def _exp2(run_root: Path) -> str:
    ...
    df, d2 = _drop_strategy_labels(df)
    _save_df(df, run_root)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--exp-id", required=True, choices=sorted(TRANSFORMS.keys()))
...
def main() -> None:
    ...
    target_col = TRANSFORMS[args.exp_id](run_root)

...
from pump_end_clustering_prod.infra.clickhouse import DataLoader
...
from pump_end_clustering_prod.pump_end.worker import (
    PumpEndWorker, PumpEndWorkerResult, PumpEndSignalState,
    detect_candidates, build_points_around_candidates,
)
from pump_end_clustering_prod.pump_end.params import DEFAULT_PUMP_PARAMS
...
class PumpEndClusteringPipeline:
    def __init__(
            self,
            tokens: list[str],
            ch_dsn: str,
        ...
        self.model = PumpEndClusteringModel(model_dir)

        self.min_candles = (
            self.model.warmup_bars
            + self.model.window_bars
            + DEFAULT_PUMP_PARAMS.liquidity_window_bars
            + 21
        )
        ...
        self.last_processed_decision_time: datetime = None

        log("INFO", "PUMP_END_CL",
            f"min_candles={self.min_candles} "
            f"(warmup={self.model.warmup_bars} + window={self.model.window_bars} "
            f"+ liq={DEFAULT_PUMP_PARAMS.liquidity_window_bars} + 21)")

    def run(self):
        if self.restore_lookback_bars > 0:
            log("INFO", "PUMP_END_CL", f"started, restoring state from last {self.restore_lookback_bars} bars")
            try:
                self._restore_state_and_cache()
            except Exception as e:
                log("ERROR", "PUMP_END_CL",
                    f"restore failed: {type(e).__name__}: {str(e)}, starting with empty state")
                self.candles_cache = {}
        else:
            log("INFO", "PUMP_END_CL", "started without state restoration")

        log("INFO", "PUMP_END_CL", "entering live mode")
        ...
        try:
            while True:
                ...
                if self._should_process(current_time):
                    ...
                    self.last_processed_decision_time = decision_open_time
                    self._process_cycle(current_time)

                sleep(0.1)
        except KeyboardInterrupt:
            log("INFO", "PUMP_END_CL", "shutting down...")
    ...
    def _should_process(self, current_time: datetime) -> bool:
        ...
        if self.last_processed_decision_time == decision_open_time:
            return False
    ...
    def _restore_symbol_state(
        ...
        candidates = detect_candidates(df, DEFAULT_PUMP_PARAMS)
        candidates = [c for c in candidates if restore_start_decision_time - timedelta(minutes=15) <= c <= restore_end_decision_time]
        ...
        restore_end_bucket = restore_end_decision_time - timedelta(minutes=15)
        points_df = points_df[points_df['open_time'] <= restore_end_bucket]
        if points_df.empty:
            return 0
        ...
        if not feature_rows or len(feature_rows) != len(valid_decision_times):
            return 0

        probas = self.model.batch_predict(feature_rows)

        valid_points = points_df.iloc[valid_indices].reset_index(drop=True)

        for cand_time in candidates:
            event_id = f"{symbol}|{cand_time.strftime('%Y%m%d_%H%M%S')}"
            event_mask = valid_points['event_id'] == event_id
            event_points = valid_points[event_mask]

            if event_points.empty:
                continue

            offset_zero_idx = event_points[event_points['offset'] == 0].index
            if len(offset_zero_idx) > 0:
                global_idx = event_mask[event_mask].index[event_points.index.get_loc(offset_zero_idx[0])]
                zero_features = feature_rows[global_idx]
            ...
            for local_i, (_, pt_row) in enumerate(event_points.iterrows()):
                global_i = list(valid_points.index).index(pt_row.name)
                p_end = probas[global_i]
                offset = int(pt_row['offset'])
                dt = valid_decision_times[global_i]

                self.signal_state.update_and_check(
                    symbol, event_id, offset, p_end, dt
                )
        ...
        with ThreadPoolExecutor(max_workers=self.workers) as executor:
            ...
            for future in as_completed(futures):
                try:
                    ...
                    log("ERROR", "PUMP_END_CL", f"token={token}USDT unexpected error: {e}")
    ...
    def _log_cycle_summary(self, results: list[PumpEndWorkerResult], cycle_duration: float, load_duration: float):
        ...
        cluster_blocked = status_counts.get("OK_CLUSTER_BLOCKED", 0)
        ...
        log("INFO", "PUMP_END_CL",
            f"cycle done ok={ok} signals_sent={signals_sent} cluster_blocked={cluster_blocked} "
            f"skip_empty={skipped_empty} "
        ...
        if errors > 0:
            ...
            log("ERROR", "PUMP_END_CL", f"token={first_error.symbol} error={first_error.error_message}")

        processed_results = [r for r in results if r.status in ("OK_NO_SIGNAL", "SIGNAL_SENT", "OK_CLUSTER_BLOCKED")]
        ...
        if processed_results:
            ...
            log("INFO", "PUMP_END_CL", f"slowest: {slowest_str}")

        p_end_values = [r.p_end for r in results if r.p_end is not None]
        if p_end_values:
            above_threshold = sum(1 for p in p_end_values if p >= self.model.global_threshold)
            max_p_end = max(p_end_values)
            log("INFO", "PUMP_END_CL",
                f"predictions above_threshold={above_threshold}/{len(p_end_values)} max_p_end={max_p_end:.4f}")

        if self.model.cluster_mode:
            cluster_results = [r for r in results if r.cluster_id is not None]
            if cluster_results:
                cluster_dist = {}
                for r in cluster_results:
                    cluster_dist[r.cluster_id] = cluster_dist.get(r.cluster_id, 0) + 1
                log("INFO", "PUMP_END_CL", f"cluster distribution: {cluster_dist}")

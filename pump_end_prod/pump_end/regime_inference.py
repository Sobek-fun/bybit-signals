...
from pump_end_prod.infra.clickhouse import DataLoader, get_liquid_universe
from pump_end_prod.infra.logging import log
from pump_end_prod.pump_end.regime_dataset import STRATEGY_STATE_MODE, build_strategy_state
...
def _resolve_inference_policy_params(guard_model_dir: Path) -> tuple[dict, str]:
    run_config = _load_json_if_exists(guard_model_dir / "run_config.json")
    if isinstance(run_config, dict):
        params = run_config.get("policy_params_used_for_inference")
        if isinstance(params, dict):
            return params, "run_config.json:policy_params_used_for_inference"

    policy_report = _load_json_if_exists(guard_model_dir / "policy_report.json")
    if isinstance(policy_report, dict):
        params = policy_report.get("policy_params_used_for_inference")
        if isinstance(params, dict):
            return params, "policy_report.json:policy_params_used_for_inference"

    policy_path = guard_model_dir / "resolved_policy_params.json"
    if policy_path.exists():
        with open(policy_path, 'r') as f:
            return json.load(f), "resolved_policy_params.json"

    policy_path = guard_model_dir / "best_policy_params.json"
    with open(policy_path, 'r') as f:
        return json.load(f), "best_policy_params.json"
...
def load_guard_artifacts(guard_model_dir: Path, ch_dsn: str) -> GuardArtifacts:
    ...
    policy_params, policy_source = _resolve_inference_policy_params(guard_model_dir)
    log("INFO", "GUARD", f"loading policy params from {policy_source}")

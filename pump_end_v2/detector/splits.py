...
def assign_detector_dataset_splits(dataset_df: pd.DataFrame, split_bounds: SplitBounds) -> pd.DataFrame:
    ...
    context_times = pd.to_datetime(frame["context_bar_open_time"], utc=True, errors="raise")
    split = pd.Series("discard", index=frame.index, dtype="string")
    train_mask = context_times <= split_bounds.train_end
    val_mask = (context_times > split_bounds.train_end) & (context_times <= split_bounds.val_end)
    test_mask = (context_times > split_bounds.val_end) & (context_times <= split_bounds.test_end)
...
def generate_detector_walkforward_folds(
    ...
    if train_df.empty:
        log_info("CV", f"fold generation summary folds_total=0 purge_gap_bars={resolver_config.horizon_bars}")
        return []
    train_df["context_bar_open_time"] = pd.to_datetime(train_df["context_bar_open_time"], utc=True, errors="raise")
    train_df = train_df.sort_values("context_bar_open_time", kind="mergesort").reset_index(drop=True)
    train_start = train_df["context_bar_open_time"].min()
    train_end = min(train_df["context_bar_open_time"].max(), pd.Timestamp(split_bounds.train_end))
    ...
    while cursor <= train_end:
        ...
        fold_val = train_df[
            (train_df["context_bar_open_time"] >= val_start) & (train_df["context_bar_open_time"] <= val_end)
        ]
        if not fold_train.empty and not fold_val.empty:
            candidate_folds.append(
                DetectorFold(
                    fold_id="",
                    train_start=fold_train["context_bar_open_time"].min().to_pydatetime(),
    ...
    log_info("CV", f"fold generation summary folds_total={len(folds)} purge_gap_bars={resolver_config.horizon_bars}")
    return folds


def filter_fold_rows(dataset_df: pd.DataFrame, fold: DetectorFold) -> tuple[pd.DataFrame, pd.DataFrame]:
    _validate_fold_inputs(dataset_df)
    train_split = dataset_df[dataset_df["dataset_split"] == "train"].copy()
    train_split["context_bar_open_time"] = pd.to_datetime(train_split["context_bar_open_time"], utc=True, errors="raise")

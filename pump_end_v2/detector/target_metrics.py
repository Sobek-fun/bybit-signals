...
def build_detector_target_metrics(policy_rows_df: pd.DataFrame, good_threshold: float = 0.5) -> dict[str, float]:
    required_columns = ("p_good", "target_good_short_now", "target_reason")
    missing = [column for column in required_columns if column not in policy_rows_df.columns]
    if missing:
    ...
    predicted_good = pd.to_numeric(frame["p_good"], errors="coerce").fillna(0.0) >= float(good_threshold)
    actual_good = pd.to_numeric(frame["target_good_short_now"], errors="coerce").fillna(0).astype(int) == 1
    ...
    return {
        ...
        "continuation_fp_rate": _reason_fp_rate(predicted_good, reasons, "continuation"),
        "flat_fp_rate": _reason_fp_rate(predicted_good, reasons, "flat"),
    }


def _reason_fp_rate(predicted_good: pd.Series, reasons: pd.Series, reason_label: str) -> float:

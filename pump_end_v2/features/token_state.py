...
def build_token_state_layer(
    df: pd.DataFrame, event_opener_config: EventOpenerConfig
) -> pd.DataFrame:
    ...
    for symbol, sdf in df.groupby("symbol", sort=False):
        parts.append(
            _build_symbol_token_state(
                symbol, sdf.reset_index(drop=True), event_opener_config
            )
        )
...
def _build_symbol_token_state(
    symbol: str, sdf: pd.DataFrame, cfg: EventOpenerConfig
) -> pd.DataFrame:
    frame = sdf.copy()
    frame["symbol"] = symbol
    ...
    frame["intrabar_range_pct"] = _safe_ratio(
        frame["high"] - frame["low"], frame["open"]
    )
    frame["candle_body_pct"] = _safe_ratio(
        frame["close"] - frame["open"], frame["open"]
    )
    frame["upper_wick_pct"] = _safe_ratio(
        frame["high"] - frame[["open", "close"]].max(axis=1), frame["open"]
    )
    frame["lower_wick_pct"] = _safe_ratio(
        frame[["open", "close"]].min(axis=1) - frame["low"], frame["open"]
    )
    frame["rolling_volatility_4"] = (
        frame["close_ret_1"].rolling(window=4, min_periods=1).std(ddof=0)
    )
    frame["rolling_volatility_12"] = (
        frame["close_ret_1"].rolling(window=12, min_periods=1).std(ddof=0)
    )
    ...
    mfi.loc[mfi_regular_mask] = 100.0 - (
        100.0 / (1.0 + money_flow_ratio.loc[mfi_regular_mask])
    )
    ...
    frame["rolling_mean_dollar_volume_12"] = (
        frame["dollar_volume"].rolling(window=12, min_periods=1).mean()
    )
    frame["dollar_volume_ratio_12"] = _safe_ratio(
        frame["dollar_volume"], frame["rolling_mean_dollar_volume_12"]
    )
    frame["liquidity_score_12"] = np.log1p(
        frame["rolling_mean_dollar_volume_12"].clip(lower=0.0)
    )
    frame["rolling_min_low"] = (
        frame["low"].rolling(window=cfg.runup_lookback_bars, min_periods=1).min()
    )
    frame["runup_pct"] = _safe_ratio(frame["close"], frame["rolling_min_low"]) - 1.0
    frame["rolling_max_high"] = (
        frame["high"].rolling(window=cfg.near_high_lookback_bars, min_periods=1).max()
    )
    frame["recent_high_distance_pct"] = _safe_ratio(
        frame["rolling_max_high"] - frame["close"], frame["rolling_max_high"]
    )
    frame["near_high_flag"] = frame["recent_high_distance_pct"] <= cfg.near_high_tol_pct
    frame["rolling_mean_volume"] = (
        frame["volume"]
        .rolling(window=cfg.volume_ratio_lookback_bars, min_periods=1)
        .mean()
    )
